...
// GET /api/clinics - Listar todas as clínicas (acessível a todos)
...
// GET /api/clinics/:id - Obter detalhes de uma clínica com suas salas (acessível a todos)
...
// POST /api/clinics - Criar uma nova clínica (admin e gerente)
router.post("/", requireRole(["admin", "gerente"]), async (req, res, next) => {
  try {
    const { name, address, city, state, zip_code, phone, host_id } = req.body;
    const result = await pool.query(
      "INSERT INTO clinics (name, address, city, state, zip_code, phone, host_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *",
      [name, address, city, state, zip_code, phone, host_id]
    );
...
// POST /api/clinics/:clinicId/rooms - Criar uma nova sala para uma clínica (admin e gerente)
